...
/*class Singelton {
	...
	Singelton(const Singelton&) = delete; //заборона створення, конструктор корій
	Singelton& operator =(const Singelton&) = delete;//заборона створення
	void* operator new(size_t) = delete;//заборона створення
	void* operator new[](size_t) = delete;//заборона створення
	void operator delete(void*) = delete;//заборона створення
	void operator delete[](void*) = delete;//заборона створення

public:
	static Singelton& getInst() {
		static Singelton obj; //створення обєкту класу
		return obj;
	}
};
